...
// Function to display sorted reservations
void afficher_reservation_triee(GtkWidget *tree_view, GtkSortType sort_order) {
    gtk_tree_view_set_model(GTK_TREE_VIEW(tree_view), NULL);
    ...
    // Array to store reservations for sorting
    reservation reservations[MAX_RESERVATIONS];
    int count = 0;

    // Read reservations from the file
    while (fscanf(file, "%s %s %s %s %s %d %d %d %d\n",
                  reservations[count].client_info.nom,
                  reservations[count].client_info.prenom,
                  reservations[count].client_info.cin,
                  reservations[count].nom_parking,
                  reservations[count].client_info.num_telephone,
                  &reservations[count].id,
                  &reservations[count].date_reservation.jour,
                  &reservations[count].date_reservation.mois,
                  &reservations[count].date_reservation.annee) == 9) {
        count++;
        if (count >= MAX_RESERVATIONS) break;  // Prevent overflow
    }
    fclose(file);

    // Sort the reservations
    if (sort_order == GTK_SORT_ASCENDING) {
        qsort(reservations, count, sizeof(reservation), compare_asc);
    } else if (sort_order == GTK_SORT_DESCENDING) {
        qsort(reservations, count, sizeof(reservation), compare_desc);
    }

    // Populate the sorted reservations into the tree view
    for (int i = 0; i < count; i++) {
        GtkTreeIter iter;
        gtk_list_store_append(store, &iter);
        gtk_list_store_set(store, &iter,
                           0, reservations[i].client_info.nom,
                           1, reservations[i].client_info.prenom,
                           2, reservations[i].client_info.cin,
                           3, reservations[i].nom_parking,
                           4, reservations[i].client_info.num_telephone,
                           5, reservations[i].id,
                           6, reservations[i].date_reservation.jour,
                           7, reservations[i].date_reservation.mois,
                           8, reservations[i].date_reservation.annee,
                           -1);
    }
    ...
    g_print("Reservations displayed in %s order.\n",
